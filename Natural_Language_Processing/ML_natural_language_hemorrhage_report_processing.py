...
class Radiology_Report_NLP(): 
    ...
    def define_apply_data(self): 
        ...
        self.reports_apply = self.reports[length_null:-1]
        self.reports_apply.drop(self.reports_apply.columns[len(self.reports_apply.columns)-1], axis=1, inplace=True)
    ...
    def report_preprocessing(self, vocab_set):
        ...
        for i in range(self.reports.shape[0]):
            sys.stdout.write('\r')
    ...
    def split_data(self):
        self.y = self.reports.loc[:][self.outcome].values
    ...
    def bayes_classifier(self):    
        ...
        self.bayes.prediction = classifier_GNB.predict(self.X_test)
        
        # Making the Confusion Matrix for the Bayesian approach
        self.Accuracy_Bayes, self.Precision_Bayes, self.Recall_Bayes, self.F1_Score_Bayes, self.cm_Bayes = self.calc_metrics(self.y_test, self.bayes.prediction)
    ...
    def random_forest_classifier(self):    
        ...
        # Predicting the Test set results
        self.RF.prediction = classifier_RF.predict(self.X_test)
    ...
    def main():
        ...
        read.bayes_classifier(X, y) 
        Accuracy.Bayes, Precision.Bayes, Recall.Bayes, F1_Score.Bayes, cm.Bayes = read.calc_metrics(self.y_test, self.bayes.prediction)
