...
def run_3d_cnn(model_arch,
    ...
    f1_score, cm, fpr, tpr, thresholds, roc_auc=Run_Utils.run_test_model(
        ...
        hdf5_file_test=hdf_file_test,
        results_path=results_path)

    network_params_dict={'model architecture': model_arch, 'batch size': batch_size,
                         'epochs': nb_epoch, 'network depth': depth, 'number layer in block': nb_dense_block,
                         'number start filters': nb_filter, 'filter growth rate': growth_rate,
                         'dropout rate': dropout_rate, 'learning rate': learning_rate, 'weight decay': weight_decay,
                         'data augmentation': data_aug, 'max number transformations': max_trans}

    results_dict={'accuracy': accuracy, 'precision': precision,
                  'recall': recall, 'f1 score': f1_score, 'auc': roc_auc,
                  'fpr': fpr, 'tpr': tpr}

    network_params_results=dict(network_params_dict.items() + results_dict.items())

    with open(results_path + 'results.csv', 'wb') as csv_file:
        writer=csv.writer(csv_file)
        for key, value in network_params_results.items():
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', type=str, help='name of the keras model name')
    parser.add_argument('--model_path', type=str, help='path for the keras models')
